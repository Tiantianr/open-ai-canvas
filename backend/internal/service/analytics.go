...
func (s *Service) EnrichAPICallLog(log *model.ApiCallLog, responseBody []byte) {
	...
	log.ProviderRequestID = firstNonEmpty(stringField(payload, "task_id"), stringField(payload, "prompt_id"), stringField(payload, "id"), stringField(payload, "request_id"), stringField(payload, "name"), log.ProviderRequestID)
