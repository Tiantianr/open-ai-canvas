...
export function focusInvalidChannelField(channel: ModelChannel) {
    ...
    const field = baseUrlError ? "base-url" : !channel.apiKey.trim() && !channelAllowsBlankApiKey(channel) ? "api-key" : requiresSecretKey(channel) && !channel.secretKey?.trim() ? "secret-key" : "models";
...
function mergeFetchedModelCosts(channel: ModelChannel, catalog: ChannelModelCatalogItem[]): ChannelModelCost[] {
    ...
    for (const item of catalog) {
        ...
        if (existing) {
            ...
            next.push(
                inferredProtocol && existing.protocol !== inferredProtocol
                    ? {
                          ...existing,
