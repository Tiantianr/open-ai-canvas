...
import { ChevronDown, Ellipsis, Link2, Lock, Plus, Unlink, Unlock } from "lucide-react";
                ...
                <div className="canvas-node-inspector" style={{ color: theme.node.text }}>
                    {view === "info" ? (
                        <div className="thin-scrollbar canvas-node-inspector-scroll">
                            ...
                            {node.type === CanvasNodeType.Image ? (
                                <section className="canvas-node-inspector-section">
                                    <div className="canvas-node-inspector-section-heading"><span>Seedance 素材</span><em>{activeSeedanceAssetId ? "已绑定" : staleSeedanceBinding ? "需重新绑定" : "未绑定"}</em></div>
                                    {readOnly ? (
                                        ...
                                        <div className="canvas-node-inspector-tag-editor">
                                            ...
                                            <Button type="primary" icon={<Link2 className="size-4" />} disabled={!seedanceAssetInput.trim()} onClick={saveSeedanceAsset}>绑定</Button>
                                            {storedSeedanceAssetId ? <Button icon={<Unlink className="size-4" />} onClick={clearSeedanceAsset}>解除</Button> : null}
                                        </div>
                                    )}
                                    {activeSeedanceAssetId ? <p className="canvas-node-inspector-help break-all">{seedanceAssetUri(activeSeedanceAssetId)}</p> : null}
                                    {staleSeedanceBinding ? <p className="canvas-node-inspector-help">图片存储已变化，请确认素材后重新绑定。</p> : null}
                                </section>
                            ) : null}
