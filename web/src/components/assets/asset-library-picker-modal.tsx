...
export function AssetLibraryPickerModal({
    ...
    const selectedIds = useMemo(() => items.filter((item) => !item.disabledReason && selected.has(item.id)).map((item) => item.id), [items, selected]);
                    ...
                    <nav className="asset-picker-categories" aria-label="素材分类">
                        {categories.map((value) => (
                            <button key={value} type="button" className={category === value ? "is-active" : ""} aria-pressed={category === value} onClick={() => setCategory(value)}>
                                <span>{categoryLabels[value] || "其他"}</span>
                        ...
                        <div className="asset-picker-grid">
                            {visibleItems.length ? (
                                ...
                                <div className="asset-picker-empty">
                                    ...
                                    <span>{upload ? "换个分类，或从底部上传一份新素材。" : emptyDescription}</span>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <>
                            ...
                            <button type="button" className="asset-picker-upload" onClick={() => uploadInputRef.current?.click()} disabled={working} aria-busy={uploading}>
                                ...
                                <span>
                                    ...
                                    <small>{uploading ? "保存完成后会自动选中" : upload.description}</small>
                                </span>
                            </button>
                        </>
                    ) : footerNote ? (
    ...
    return (
        <button type="button" className={cn("asset-picker-card", selected && "is-selected", disabled && "is-disabled")} onClick={onToggle} disabled={disabled} aria-pressed={selected} title={item.disabledReason || item.title}>
            <div className="asset-picker-card-media">
                {item.imageUrl ? (
                    <img src={item.imageUrl} alt={item.title} loading="lazy" decoding="async" className={item.imageFit === "contain" ? "is-contain" : undefined} />
                ) : (
                    <AssetMediaPreview asset={item.asset} alt={item.title} fallback={<div className="asset-picker-card-fallback">{kindIcon(item.kindLabel)}</div>} />
                )}
                <span className="asset-picker-card-check">
                    <Check />
                </span>
